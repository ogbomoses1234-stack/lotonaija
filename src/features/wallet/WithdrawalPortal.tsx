...
/**
 * Withdrawal form styled to match Play page premium aesthetic
 */
export const WithdrawalPortal = memo(() => {
  const { actions, isProcessing } = useWalletStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setShowToast(true);
    const success = await actions.withdraw(bankCode, nuban, parseFloat(amount) || 0);
  ...
  const canSubmit = amount && parseFloat(amount) > 0 && bankCode && nuban.length === 10 && !isProcessing;

  return (
    <GlassCard className="p-6 space-y-6 bg-black/20 border-white/5 zigzag-bottom relative overflow-hidden">
      {/* Ambient glow */}
      ...
      <div className="relative z-10 space-y-2">
        ...
        <p className="text-xs text-white/50">Instant processing to verified accounts</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
        ...
        <div className="p-3 rounded-xl bg-white/[0.03] border border-white/10 flex items-start gap-3">
          ...
          <p className="text-xs text-white/60">
            Withdrawals are processed within <span className="text-white font-medium">24-48 hours</span>. 
        ...
        <PrimaryButton 
          ...
          className={cn(
            ...
            canSubmit && "bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500"
          )}
        >
          {isProcessing ? (
